	 ...
	 if (!empty($persona_nombre))
	  {
	   if ($persona_tipo=='F')
	     ...
	     {
		  // Consulta persona jur�dica por nombre
		  $_tipoconsulta=5;
		 }
	   if (strlen($persona_nombre)<3)
	    {
		 $_tipoconsulta=0;
		 $_mensaje.='<br>� Debe escribir m�s de 2 caracteres en el nombre !';
		}
	 ...
	 if (!empty($persona_dni_cuit))
	  {
	   if ($persona_tipo=='F')
	     ...
	     {
		  // Consulta persona jur�dica por cuit
		  $_tipoconsulta=7;
		 }
	  }
...
switch($_tipoconsulta)
 {
  ...
  case '4':
   // Busca persona F�sica ingresada en variable $persona_nombre
   $result=$mysqli->query("SELECT * FROM _personas
   ...
   $result=$mysqli->query("SELECT * FROM _personas
	                          ...
	                          ORDER BY persona_nombre
						      LIMIT $desde,$maxmostrar");
   $subtitulo='(Persona F�sica consultada: '.$persona_nombre.')';
   break;

  case '5':
   // Busca persona Jur�dica ingresada en variable $persona_nombre
   $result=$mysqli->query("SELECT * FROM _personas
   ...
   $result=$mysqli->query("SELECT * FROM _personas
	                          ...
	                          ORDER BY persona_nombre
						      LIMIT $desde,$maxmostrar");
   $subtitulo='(Persona Jur�dica consultada: '.$persona_nombre.')';
   ...
   if ($_accion=="MQ")
     {
      ...
      // Muestro link "Quitar"   
      $AuxPregunta="'� Est� seguro de QUITAR Licitaci�n N� ".$row["numero"].' ('.$row["proyecto_nombre"].") ?'";
